...
﻿			<h1 class="grid_12">Đơn đặt hàng</h1>
			
			<div class="grid_12">
				<div class="box">
					<div class="header"><h2>Chi tiết đơn đặt hàng</h2></div>
					<div class="content invoice">
					
						<h4>Số đơn hàng: <?php echo $orderid;?></h4>
						<h3>Đơn đặt hàng trực tuyến</h3>
						<div class="supplier">
							<h5>Địa chỉ thanh toán:</h5>
							<p>
								<strong><?php echo sitesystem::getInstance()->get_user_info($invoice->uid,"fullname");?></strong><br />
							</p>
							<h5>Địa chỉ:</h5>
							<p>
                                <?php echo sitesystem::getInstance()->get_user_info($invoice->uid,"address");?><br />
								Việt Nam
							</p>
						</div>
						<div class="customer">
							...
							<p>
								<strong><?php echo $invoice->shipping_name;?></strong><br />
								<?php echo $invoice->shipping_address;?><br />
							</p>
							<p>
								<span>ID:</span> null<br />
								<span>Phone:</span> <?php echo $invoice->shipping_phone;?>
							</p>
						</div>
						...
						<div class="details">
							<div class="highlighted">
								<span>Mã tài khoản:</span> <strong><?php echo $invoice->uid;?></strong>
							</div>
							<div>
								<span>Ngày đặt hàng:</span> <strong><?php echo $invoice->orderaddtime;?></strong>
							</div>
							<div>
								<span>Ngày thanh toán:</span> <strong><?php echo $invoice->paymentdate;?></strong>
							</div>
						</div>
						
						<table class="styled borders">
							<thead>
								<tr><th>Sản phẩm</th><th>Mô tả</th><th>Thành tiền</th></tr>
							</thead>
							<tbody>
                            <?php
                            $listproduct = order::getInstance()->get_product_in_invoice($orderid);
                            foreach($listproduct as $product)
                            {
                                ?>
                                <tr><td style="width: 30%;"><?php echo product::getInstance()->get_product_info($product->productid,"product_title");?> (Số lượng: <?php echo $product->productqty;?>)</td><td style="width: 60%;"><?php echo substr(strip_tags(product::getInstance()->get_product_info($product->productid,"product_description")),0,400);?></td><td><?php echo number_format($product->productprice, 0, ',', '.');?> </td></tr>
                            <?php
                            }
                            ?>
						...
						<div class="total">
							<span>Tổng cộng: </span><strong><?php echo number_format(order::getInstance()->sumarybill($orderid), 0, ',', '.');?> VNĐ</strong>
						</div>
						...
						<div class="notes">
							<h5>Ghi chú:</h5>
							<p>Các chính sách bảo hành theo quy định của nhà sản xuất, chi phí bảo hành không bao gồm thành tiền trong hóa đơn này. Các khuyến mãi sẽ được cộng trực tiếp và hóa đơn</p>
						</div>
