...
<body>
	...
	<div style="display: none;" id="dialog_add_client" title="Thêm danh mục mới">
		<form action="" method="POST" class="full validate" />
			<div class="row">
				...
				<div>
					<input class="required" type="text" name="catname" id="catname" />
					<input type="hidden" value="add" name="catadd" id="catadd" />
				</div>
			</div>
			<div class="row">
						<label for="catparent">
						...
						<div>
							<select name="catparent" id="catparent" class="search" data-placeholder="Chọn danh mục gốc">
								<option value=""></option>
                                <?php
                                $catlist = product::getInstance()->get_category("*");
                                foreach($catlist as $catt)
                                {
                                    ?>
                                    <option value="<?php echo $catt->catid;?>"><?php echo $catt->catname;?></option>
                                <?php
                                }
                                ?>
...
﻿			<h1 class="grid_12">Quản lý danh mục sản phẩm</h1>
				...
				<div class="box">
					...
					<div class="content">
						...
						<div class="tabletools">
							<div class="left">
								<a class="open-add-client-dialog" href="javascript:void(0);"><i class="icon-plus"></i>Thêm danh mục mới</a>
							</div>
							<div class="right">
						...
						<table class="styled">
							...
							?>
								<tr>
									...
									<td><?php echo  product::getInstance()->get_category_info($cat->catparent);?></td>
									<td class="center">
										<a href="<?php echo XC_URL;?>/admin/category/?action=edit&catid=<?php echo $cat->catid;?>" class="button small grey tooltip" data-gravity=s title="Sửa"><i class="icon-pencil"></i></a>
										<a href="<?php echo XC_URL;?>/admin/category/?action=delete&catid=<?php echo $cat->catid;?>" class="button small grey tooltip" data-gravity=s title="Xóa"><i class="icon-remove"></i></a>
									</td>
								</tr>
								<?php }?>
							</tbody>
			...
			<div class="grid_12">
				...
				<fieldset>
					<div class="row">
						...
						<div>
							<input type="text" value="<?php echo $catid;?>" disabled name="catids" id="catids" />
						</div>
					</div>
					<div class="row">
						<label for="catname">
						...
						<div>
							<input type="text" value="<?php echo $catname;?>" name="catname" id="catname" />
							<input type="hidden" value="update" name="catupdate" id="catupdate" />
							<input type="hidden" value="<?php echo $catid;?>" name="catid" id="catid" />
						</div>
					</div>
					<div class="row">
						<label for="catparent2">
							<strong>Danh mục gốc</strong>
						</label>
						<div>
							<select name="catparent2" id="catparent2" class="search" data-placeholder="<?php echo product::getInstance()->get_category_info($catparent);?>">
								<option value=""></option>
                                <?php
                                $catlist = product::getInstance()->get_category(0);
                                foreach($catlist as $catt)
                                {
                                    ?>
                                    <option value="<?php echo $catt->catid;?>"><?php echo $catt->catname;?></option>
